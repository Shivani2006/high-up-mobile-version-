...
function setup(){
  ...
  balloon = createSprite(200,height/2);
  balloon.addImage(balloonImg);
  balloon.scale = 0.4;
  //balloon.debug = true;
  balloon.setCollider("circle",0,-80,210)
 
  trophy = createSprite(width/3,50);
  trophy.addImage(trophyImg);
  trophy.scale = 0.4;
  trophy.visible = false;
  
  trophy2 = createSprite(width/2.5,50);
  trophy2.addImage(trophy2Img);
  trophy2.scale = 0.5;
  trophy2.visible = false;
  
  trophy3 = createSprite(width/2,50);
  ...
  upperEdge = createSprite(width/2,20,width,20);
...
function spawnGoldCoins() {
  ...
  if (frameCount % 120 == 0) {
    goldCoin = createSprite(width,50,10,10);
    goldCoin.y = Math.round(random(20,height-150));
...
function spawnObstacles() {
  if (frameCount % 80 == 0) {
    var obstacle = createSprite(width,400,10,10);
    obstacle.y = Math.round(random(20,height-120));
...
function spawnSilverCoins() {
  //write code here to spawn the bones
  if (frameCount % 150 == 0) {
    silverCoin = createSprite(width,50,40,10);
    silverCoin.y = Math.round(random(20,height-120));
